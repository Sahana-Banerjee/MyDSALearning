...
public static boolean compareArray(final char[] array1, final char[] array2 ){
  ...
  //Optimised code using for loop with array since Arrays are contiguous memory spaces. Data can be obtained through indexes. The time complexity is O(1), so it is fast.
...
//All of the above solutions have time complexity of O(n^2), hence better solution
static boolean compareArray2(final char[] array1, final char[] array2 ){
  ...
  return false;
  //Here this method operates in for loop separately over the given arrays and the O(m+n)~O(n)
